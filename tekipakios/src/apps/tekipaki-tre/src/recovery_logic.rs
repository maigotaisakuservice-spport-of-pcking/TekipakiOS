...
pub fn find_datalocker_partition() -> Option<String> {
    let output = Command::new("lsblk")
        .args(["-no", "PATH,LABEL"])
        .output()
        .ok()?;
    ...
    for line in stdout.lines() {
        let parts: Vec<&str> = line.split_whitespace().collect();
        if parts.len() >= 2 && parts[1] == "TEKIPAKI_DATALOCKER" {
            return Some(parts[0].to_string());
        }
    }
    None
...
pub fn check_system_integrity() -> Result<(), String> {
    ...
    // We also include 'archlinux-keyring' to ensure the database is up-to-date
    let status = Command::new("pacman")
        .args(["--sysroot", &mnt, "-Syu", "--noconfirm", "archlinux-keyring", "base", "linux-tekipaki", "grub", "tekipaki-guardd"])
        .status();

    if !status.map(|s| s.success()).unwrap_or(false) {
        return Err("Package repair failed / パッケージの修復に失敗しました".to_string());
    }

    // 3. Re-install Bootloader (GRUB)
    let grub_install = Command::new("arch-chroot")
        .args([&mnt, "grub-install", "--target=x86_64-efi", "--efi-directory=/boot", "--bootloader-id=Tekipaki"])
        .status();

    let grub_config = Command::new("arch-chroot")
        .args([&mnt, "grub-mkconfig", "-o", "/boot/grub/grub.cfg"])
        .status();

    match (grub_install, grub_config) {
        (Ok(s1), Ok(s2)) if s1.success() && s2.success() => Ok(()),
        _ => Err("Bootloader repair failed / ブートローダーの修復に失敗しました".to_string()),
    }
}
...
pub fn cloud_reinstall() -> Result<(), String> {
    ...
    if !check_internet() {
        return Err("No internet connection detected / インターネット接続が見つかりません".to_string());
    }
    ...
    let status = Command::new("pacstrap")
        .args(["/mnt", "base", "linux-tekipaki", "linux-firmware", "plasma-desktop", "grub", "efibootmgr", "tekipaki-guardd"])
        .status();
