...
if [ "$CI" = "true" ] || [ "$EUID" -eq 0 ]; then
    pacman -Sy --noconfirm --needed \
        base-devel archiso git bc libelf pahole cpio perl tar xz \
        rust gtk4 pkgconf mkinitcpio kmod mkinitcpio-archiso \
        mkinitcpio-nfs-utils linux-firmware arch-install-scripts \
        binutils openssl flex bison grub libisoburn mtools \
        dosfstools python nbd pv syslinux sudo mtd-utils archlinux-appstream-data \
        dbus-glib vte3 libnotify appstream appstream-glib
fi
...
mkdir -p "$KERNEL_DIR"
git clone --depth 1 https://github.com/damentz/zen-kernel.git "$KERNEL_DIR"
cd "$KERNEL_DIR"
...
# Dependencies for MTD, NBD and DRM Privacy Screen
...
# Ensure versioning is clean
./scripts/config --set-str CONFIG_LOCALVERSION "-tekipaki"
./scripts/config --disable CONFIG_LOCALVERSION_AUTO

# Fix "+" suffix and git hash in version
touch .scmversion
export LOCALVERSION=""
...
# Verify critical modules were built
echo "Verifying modules..."
for mod in phram nbd; do
    if find "$KERNEL_DIR" -name "${mod}.ko*" | grep -q .; then
        echo "Module $mod found."
    else
        echo "Error: Module $mod not found in build directory!"
        # We don't exit here yet to see if they are installed, but this is a bad sign.
    fi
done
...
# Verify installation to root
if [ -d "/lib/modules/$KVER" ]; then
    echo "Modules installed to /lib/modules/$KVER"
    find "/lib/modules/$KVER" -name "phram.ko*"
    find "/lib/modules/$KVER" -name "nbd.ko*"
else
    echo "Error: Modules not found in /lib/modules/$KVER"
    exit 1
fi
...
mkdir -p "$PROFILE_DIR/airootfs/etc/systemd/system/multi-user.target.wants"
mkdir -p "$PROFILE_DIR/airootfs/etc/systemd/system/timers.target.wants"
# Ensure the parent directory for cp exists
...
build_aur_pkg() {
    ...
    # We use -Sy to ensure the local database created above is recognized
    ...
    # Ensure we use the correct db name
    repo-add "$PROFILE_DIR/repo/tekipaki-local.db.tar.gz" "$PROFILE_DIR/repo/"*.pkg.tar.zst
    # Refresh pacman to see the new package
    pacman -Sy
}
...
mkdir -p "$PROFILE_DIR/repo"
if [ ! -f "$PROFILE_DIR/repo/tekipaki-local.db.tar.gz" ]; then
    # We use tekipaki-local to match the pacman.conf entry
    tar czf "$PROFILE_DIR/repo/tekipaki-local.db.tar.gz" -T /dev/null
fi
...
# Initial sync for the new local repo
pacman -Sy

# Build in order to satisfy dependencies
# archlinux-appstream-data-pamac is deprecated/404, using official package instead
# build_aur_pkg "archlinux-appstream-data-pamac"

# Re-enabling Pamac AUR components
